...
**  
Observations**: - Write your observations here! - My “good” and “fair”
observations are pretty similar, while my “poor” observations average
out to a value that deviates considerably more. Why might your table
differ from Michelson’s? Michelson’s seems to round to the nearest ten?
There a number of reasons why you might want to do this; ease of
display, ease of calculation; I don’t necessarily think it makes either
table *wrong*, per se, just different.
...
``` r
diff <- abs(LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON) - LIGHTSPEED_PM
error_est <- LIGHTSPEED_PM/LIGHTSPEED_MICHELSON
error_act <- abs(LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON)/LIGHTSPEED_MICHELSON
...
of his error is less than the true error. We can tell this is true
because `LIGHTSPEED_PM` is less than the actually error between
`LIGHTSPEED_VACUUM` and `LIGHTSPEED_MICHELSON.`-Make a quantitative
comparison between Michelson’s uncertainty and his error. Michelson’s
uncertainty implied that there was less than 0.02% error in his
estimation of the speed of light; however, there was actually about a
0.05% error in his estimate of the speed of light (as compared to light
in a vacuum).
...
**Observations**: Similarities - (Both of these charts appear to exist
roughly in the same range of data (spending a majority of the time
between 299900 and 300000, which makes sense) Differences - (Simulated
data appears to generally be less spread out, while real data often hops
from extremely to extreme, even on the same day; interestingly, the real
data appeared to better conform to the velocity boundaries as time went
on, while the simulated data had the opposite behavior. There’s probably
not a causal link here, but still interesting to observe.)
...
df_q2 %>%
  ...
  ggplot(aes(color = source, Temp, velocity)) +
...
df_q2 %>%
  ...
  ggplot(aes(color = source, Distinctness, velocity)) +
...
- Honestly, I’m not certain what observations I ought to make. It
  doesn’t seem like either temperature nor distinctness tells me
  anything major regarding the real and simulated data. We still kind of
  see that all of the major outliers exist in the real dataset when
  looking at the temperature graph (which still checks out; the real
  world is randomly weird), but the trend is not so obvious when we look
  at Distinctness. I don’t observe any major trends that would tell me
  whether or not the data belongs to the “real” family or the “simulated
  family”.
