...
-   Oof. Lots of variables here. It looks like there's 4 repetitions of the idx, where they contain the same values in every input column, but getting different outputs (which makes sense)
-   We've run this simulation 35 times, at 4 different locations each time.
-   On every run, ALL of the inputs change. That could potentially be a problem if we're trying to determine causation.
...
df_psaap %>%
  ...
  # geom_text(aes(label = idx), hjust = 3, vjust = -1) 
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   It seems like the cheat fit is the most accurate, and the nonphysical fit is the least accurate.
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is classified as an output variable. It's an effective predictor because it and T_norm are both affected by the same variable, thus making it likely that they would correlate.
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   No. Again, because avg_T is an output variable, we'd only be able to get it if we gave the appropriate inputs and therefore produced T_norm
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is classified as a metadata variable, and it doesn't have any physical meaning (it just organizes the trials).
